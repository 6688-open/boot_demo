 ...
 * 
 * @author: wj
 * @version: 2019-08-28 16:56:03
 */
public class UniversalTimeUtil {
	 ...
	 * @return String
	 */
	public static String getUniversalTime(String format) {
		String formatDate = format(getUniversalTime(), format);
		return formatDate;
	}
	 ...
	 * @return
	 */
	public static String getUniversalTimeByLocalTime(String localDate, String format) {
		Calendar cal = Calendar.getInstance();
		// 获得时区和 GMT-0 的时间差,偏移量
		int offset = cal.get(Calendar.ZONE_OFFSET);
		// 获得夏令时 时差
	 ...
	 * @return
	 */
	public static Date getUniversalTimeByLocalTimeByDate(String localDate, String format) {
		Calendar cal = Calendar.getInstance();
		// 获得时区和 GMT-0 的时间差,偏移量
		int offset = cal.get(Calendar.ZONE_OFFSET);
		// 获得夏令时 时差
		int dstoff = cal.get(Calendar.DST_OFFSET);
		// 获得当前是UTC时区的时间毫秒值
		return new Date(getMillSecond(format, localDate) - (offset + dstoff));
	}
