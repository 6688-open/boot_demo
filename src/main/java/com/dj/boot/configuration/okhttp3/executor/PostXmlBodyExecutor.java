...
import com.dj.boot.configuration.okhttp3.serializer.support.DefaultXmlParamsSerializer;
...
public class PostXmlBodyExecutor implements HttpExecutor {
    private static final Logger log = Logger.getLogger(PostJsonBodyExecutor.class);
    private static final ParamsSerializer SERIALIZER = new DefaultXmlParamsSerializer();
    ...
    public byte[] execute(HttpExecutionContext context) throws Exception {
        ...
        final String xml = serializer.serialize(context.params());
        final RequestBody body = RequestBody.create(MediaType.parse("text/xml"), xml);
